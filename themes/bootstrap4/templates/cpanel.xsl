...
<xsl:template match="content[@id='cpanel-setting' and @type='normal']" priority="1">
<div class="input-group js-setting">
  <xsl:element name="input">
    <xsl:attribute name="class">form-control js-value</xsl:attribute>
    <xsl:choose>
      <xsl:when test="@subtype='number'"><xsl:attribute name="type">number</xsl:attribute></xsl:when>
      <xsl:when test="@subtype='datetime'"><xsl:attribute name="type">datetime-local</xsl:attribute></xsl:when>
