...
class MeLeeCMS
{
   ...
   */
   ...
   @param string $subtheme Any subtheme, which will use a different XSL file if one exists for that subtheme.
   ...
   */
	public function parseTemplate($data, $class, $subtheme, $xsl_includes=[])
	{
      $page_xsl = [];
	...
	public function render($subtheme="")
	{
      ...
      foreach($this->page_content as $tag=>$content)
      {
		...
		{
			$html = $this->parseTemplate($params, "MeLeeCMS", $subtheme, $content_xsl);
			if(is_array($html))
