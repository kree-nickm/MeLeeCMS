...
class MeLeeCMS
{
  ...
  /** @var string The format of the full title that will appear on the browser window, can be set by {@see MeLeeCMS::setTitle()}. */
  protected $title = "";
  /**
  ...
  */
  ...
  protected $page_xsl = [];
  /** @var array<string,Content\Content> The various objects that are subclasses of Content, which will be used to display the page content. Generally added by {@see MeLeeCMS::addContent()}. */
  ...
  */
  public function __construct($mode=self::MODE_ALL)
  {
    ...
    $this->title = $this->settings['title_format'] = $GlobalConfig['title_format'];
  ...
  */
  ...
  public function findPage($path_info=[], $args=[])
  {
    ...
    else
    {
      ...
      if(!empty($this->page))
      {
        ...
        if(empty($req_perms) || !empty($this->user) && $this->user->hasPermission(...$req_perms))
        {
          ...
          $this->page->args = empty($args) ? [] : $args;
        }
  ...
  {
    return $this->title = (string)$title;
  }
  ...
  @param array $data Multi-dimensional array of content data generated by {@see Content\Content::build_params()}
  ...
  */
  ...
  public function render($format="")
  {
    ...
    $params = [
      'title' => str_replace(["%PT%", "%ST%"], [$this->page->title, $this->getSetting('site_title')], $this->title),
