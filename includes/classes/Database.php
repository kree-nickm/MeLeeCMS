...
class Database
{
  ...
  public function insert($table, $mysql_data, $update=true, $leave_cols=[], $log=false)
  {
    ...
    if($log && $success)
    {
      ...
      {
        trigger_error("Insert was successful ({$success}), but cannot update change log because affected_ids array is somehow empty. Last ID: {$last_id}; Previous: ".implode(",",$previous), E_USER_WARNING);
      }
    }
    return $success;
  }
